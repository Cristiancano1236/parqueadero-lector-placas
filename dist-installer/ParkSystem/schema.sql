...
-- Tabla de Tipos de Vehículos (dinámica por empresa)
CREATE TABLE tipos_vehiculos (
    ...
    fecha_creacion DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_empresa) REFERENCES empresas(id_empresa),
    CONSTRAINT uq_tipo_empresa UNIQUE (codigo, id_empresa),
    INDEX idx_empresa_tipo (id_empresa, activo)
) ENGINE=InnoDB;
...
CREATE TABLE capacidades_tipo (
    ...
    CONSTRAINT uq_capacidad_tipo UNIQUE (id_empresa, id_tipo)
) ENGINE=InnoDB;
...
-- Tabla de Mensualidades (suscripciones por vehículo o arriendos de espacio)
CREATE TABLE mensualidades (
    id_mensualidad INT AUTO_INCREMENT PRIMARY KEY,
    id_empresa INT NOT NULL,
    id_vehiculo INT NULL,
    titular_nombre VARCHAR(120) NOT NULL,
    titular_documento VARCHAR(40),
    titular_telefono VARCHAR(30),
    titular_email VARCHAR(120),
    ...
    tipo_servicio ENUM('mensualidad','arriendo') NOT NULL DEFAULT 'mensualidad',
    referencia_espacio VARCHAR(100),
    observaciones VARCHAR(255),
    fecha_creacion DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (id_empresa) REFERENCES empresas(id_empresa),
    FOREIGN KEY (id_vehiculo) REFERENCES vehiculos(id_vehiculo),
    INDEX idx_mens_emp_veh (id_empresa, id_vehiculo),
    INDEX idx_mens_estado (id_empresa, estado)
) ENGINE=InnoDB;

-- Tabla de Pagos de Mensualidades
CREATE TABLE mensualidades_pagos (
    id_pago_mensualidad INT AUTO_INCREMENT PRIMARY KEY,
    id_empresa INT NOT NULL,
    ...
    id_usuario INT NOT NULL,
    FOREIGN KEY (id_empresa) REFERENCES empresas(id_empresa),
    FOREIGN KEY (id_mensualidad) REFERENCES mensualidades(id_mensualidad),
    FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario),
    INDEX idx_mp_emp_fecha (id_empresa, fecha_pago)
) ENGINE=InnoDB;
...
-- Tarifas de ejemplo por empresa 1 (usando id_tipo en lugar de tipo_vehiculo)
