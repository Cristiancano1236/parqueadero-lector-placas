...
async function getEmpresaInfo() {
    ...
    // Cargar config (reglamento) para incluirlo en el ticket
    try {
        const cr = await fetch('/api/empresa/config', { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
        const cj = await cr.json();
        if (cr.ok && cj.data && __empresaInfo) {
            __empresaInfo.reglamento = cj.data.reglamento || null;
        }
    } catch (_) {}
...
function updateDashboardStats(data) {
    ...
    (data.currentVehiclesByType || []).forEach(r => {
        const key = (r.tipo_codigo || r.tipo || '').toLowerCase();
        if (key in map) map[key] = Number(r.count || 0);
    });
    const total = map.carro + map.moto + map.bici;
    const elTotal = document.getElementById('currTotal');
    if (elTotal) elTotal.textContent = total;
    document.getElementById('currCarros').textContent = map.carro;
    document.getElementById('currMotos').textContent = map.moto;
    document.getElementById('currBicis').textContent = map.bici;

    // Ingresos totales del día (parqueo + mensualidades)
    const incParqueo = Number(data.todayIncome || 0);
    const incMens = Number(data.todayIncomeMensualidades || 0);
    document.getElementById('todayIncome').textContent = formatCurrency(incParqueo + incMens);
    const elParqueo = document.getElementById('todayIncomeParqueo');
    const elMensEl = document.getElementById('todayIncomeMens');
    if (elParqueo) elParqueo.textContent = formatCurrency(incParqueo);
    if (elMensEl) elMensEl.textContent = formatCurrency(incMens);

    // KPIs mensualidades
    const elActivas = document.getElementById('mensActivas');
    const elVencidas = document.getElementById('mensVencidas');
    const elProximas = document.getElementById('mensProximas');
    if (elActivas) elActivas.textContent = data.mensualidadesActivas || 0;
    if (elVencidas) elVencidas.textContent = data.mensualidadesVencidas || 0;
    if (elProximas) elProximas.textContent = data.mensualidadesProximas || 0;
...
async function setOcupacionKpi(){
    try{
        ...
        const bar = document.getElementById('ocupacionBar');
        if (bar) {
            bar.style.width = `${ocup}%`;
            bar.className = 'progress-bar ' + (ocup >= 90 ? 'bg-danger' : ocup >= 70 ? 'bg-warning' : 'bg-success');
        }
    }catch(_){
        const el = document.getElementById('kpiOcupacionDash');
...
function renderTicketSalida(salida, empresa){
    ...
    // Reglamento/condiciones del servicio (opcional, al pie del ticket)
