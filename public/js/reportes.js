...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Relacionado con: src/routes/tipos-vehiculos.js para tipos dinámicos
    cargarTiposVehiculos().then(() => cargarTodo());

    // Filtro mensualidades
    document.getElementById('btnFiltrarMens')?.addEventListener('click', () => { __pageMens = 0; cargarMensualidadesPagos(); });
    document.getElementById('fMensQ')?.addEventListener('keydown', e => { if (e.key === 'Enter') { __pageMens = 0; cargarMensualidadesPagos(); } });
    document.getElementById('btnMensPrev')?.addEventListener('click', () => { if (__pageMens > 0) { __pageMens--; cargarMensualidadesPagos(); } });
    document.getElementById('btnMensNext')?.addEventListener('click', () => { __pageMens++; cargarMensualidadesPagos(); });
...
// Cargar tipos de vehículos dinámicamente para el filtro
// Relacionado con: src/routes/tipos-vehiculos.js para tipos dinámicos
async function cargarTiposVehiculos() {
    try {
        ...
        if (!res.ok) throw new Error(j.message || 'Error cargando tipos');
        
        const select = document.getElementById('fTipo');
        if (!select) return;
        ...
        if (j.success && j.data) {
            j.data.forEach(tipo => {
                ...
                option.value = tipo.codigo; // Usar código para el filtro
...
function initTurnosView(){
    ...
    function printHtmlAtWidth(html, mm){
      ...
      localStorage.setItem(key, JSON.stringify({ html: html, titulo: 'Reimpresión Cierre', width: mm || 80, qrPayload: null }));
      const w = window.open('/admin/print.html?key=' + encodeURIComponent(key), '_blank');
      if (!w) { alert('Por favor permite las ventanas emergentes para imprimir.'); }
    }
...
async function cargarKPIs(){
    try{
        ...
        const elMov = document.getElementById('kpiIngresosMov');
        const elMens = document.getElementById('kpiIngresosMens');
        if (elMov) elMov.textContent = formatCurrency(d.ingresosMov||0);
        if (elMens) elMens.textContent = formatCurrency(d.ingresosMens||0);
...
window.reimprimirSalida = async function(idMovimiento){
    try{
        ...
        const _key = 'pj_' + Date.now();
        localStorage.setItem(_key, JSON.stringify({ html: header + body, titulo: 'Reimpresión', width: 80, qrPayload: null }));
        const printWin = window.open('/admin/print.html?key=' + encodeURIComponent(_key), '_blank');
        if (!printWin) { alert('Por favor permite las ventanas emergentes para imprimir.'); }
    }catch(e){ toast('Error', e.message, 'error'); }
}
...
// ── Mensualidades: pagos del período ──────────────────────────────────────

async function cargarMensualidadesPagos() {
    const token = localStorage.getItem('token');
    const { desde, hasta } = paramsBase();
    const tipoServicio = document.getElementById('fMensTipo')?.value || '';
    const q = document.getElementById('fMensQ')?.value.trim() || '';

    const params = new URLSearchParams({
        desde, hasta,
        page: String(__pageMens),
        pageSize: String(__pageSizeMens)
    });
    if (tipoServicio) params.append('tipo_servicio', tipoServicio);
    if (q) params.append('q', q);

    const tb = document.getElementById('tbMensPagos');
    if (!tb) return;

    try {
        const res = await fetch(`/api/reportes/mensualidades-pagos?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const j = await res.json();
        if (!res.ok) throw new Error(j.message || 'Error cargando pagos');

        const rows = j.data || [];
        if (rows.length === 0) {
            tb.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-3">Sin pagos en el período</td></tr>';
        } else {
            tb.innerHTML = rows.map(r => `
                <tr>
                    <td>${formatDateOnly(r.fecha)}</td>
                    <td><span class="badge bg-light text-dark border">${r.placa || r.referencia_espacio || '-'}</span></td>
                    <td>
                        <div class="fw-semibold">${r.titular_nombre || ''}</div>
                    </td>
                    <td>${r.tipo_servicio === 'arriendo'
                        ? '<span class="badge bg-info text-dark">Arriendo</span>'
                        : '<span class="badge bg-primary">Mensualidad</span>'}</td>
                    <td><span class="small text-muted">${formatDateOnly(r.periodo_desde)} — ${formatDateOnly(r.periodo_hasta)}</span></td>
                    <td><span class="badge bg-light text-dark border">${r.metodo_pago || ''}</span></td>
                    <td class="text-end fw-semibold">${formatCurrency(r.monto || 0)}</td>
                </tr>
            `).join('');
        }

        // Total acumulado
        const elTotal = document.getElementById('mensPagosTotalMonto');
        if (elTotal) elTotal.textContent = formatCurrency(j.totalMonto || 0);

        // Paginación
        const total = j.paging?.total || 0;
        const maxPage = Math.max(0, Math.ceil(total / __pageSizeMens) - 1);
        const pageInfo = document.getElementById('mensPagosPageInfo');
        if (pageInfo) pageInfo.textContent = `Página ${__pageMens + 1} de ${maxPage + 1} (${total} registros)`;
        const btnPrev = document.getElementById('btnMensPrev');
        const btnNext = document.getElementById('btnMensNext');
        if (btnPrev) btnPrev.disabled = __pageMens <= 0;
        if (btnNext) btnNext.disabled = __pageMens >= maxPage;

    } catch (e) {
        tb.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-3">${e.message}</td></tr>`;
    }
}
