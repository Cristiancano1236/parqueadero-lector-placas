...
// Valida tipo de vehículo (ahora acepta códigos de tipos dinámicos)
// Relacionado con: src/routes/tipos-vehiculos.js para tipos dinámicos
// Nota: Ya no validamos contra lista hardcodeada, aceptamos cualquier código válido
function toSafeTipoVehiculo(value) {
  if (typeof value !== 'string') return null;
  // Normalizar: minúsculas, sin espacios, solo letras, números y guiones bajos
  const v = value.toLowerCase().trim().replace(/\s+/g, '_');
  // Validar formato: solo letras, números y guiones bajos, máximo 20 caracteres
  if (!/^[a-z0-9_]{1,20}$/.test(v)) return null;
  return v;
}
