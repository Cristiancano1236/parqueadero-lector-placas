...
// GET /api/reportes/mensualidades-pagos
// Pagos de mensualidades filtrados por fecha, tipo_servicio y búsqueda (placa / titular)
router.get('/mensualidades-pagos', sanitizeReportFilters, async (req, res) => {
  try {
    const idEmpresa = req.user.id_empresa;
    const { desde: dDesde, hasta: dHasta, page, pageSize } = req.sanitized;
    const tipoServicio = req.query.tipo_servicio || '';
    const q = req.query.q ? String(req.query.q).trim() : '';

    const where = ['mp.id_empresa = ?', 'DATE(mp.fecha_pago) BETWEEN ? AND ?'];
    const params = [idEmpresa, dDesde, dHasta];

    if (tipoServicio && ['mensualidad', 'arriendo'].includes(tipoServicio)) {
      where.push('mm.tipo_servicio = ?');
      params.push(tipoServicio);
    }
    if (q) {
      where.push('(v.placa LIKE ? OR mm.referencia_espacio LIKE ? OR mm.titular_nombre LIKE ?)');
      const like = `%${q}%`;
    ...
    const [rows] = await pool.query(
      `SELECT mp.id_pago_mensualidad, DATE(mp.fecha_pago) AS fecha, mp.metodo_pago, mp.monto,
              mp.periodo_desde, mp.periodo_hasta, mp.referencia_pago,
              COALESCE(v.placa, mm.referencia_espacio) AS placa,
              mm.titular_nombre, mm.tipo_servicio, mm.id_mensualidad, mm.referencia_espacio
       FROM mensualidades_pagos mp
       JOIN mensualidades mm ON mm.id_mensualidad = mp.id_mensualidad AND mm.id_empresa = mp.id_empresa
       LEFT JOIN vehiculos v ON v.id_vehiculo = mm.id_vehiculo
       ${whereSql}
    ...
    const [[sumRow]] = await pool.query(
      `SELECT COALESCE(SUM(mp.monto), 0) AS total_monto
       FROM mensualidades_pagos mp
       JOIN mensualidades mm ON mm.id_mensualidad = mp.id_mensualidad AND mm.id_empresa = mp.id_empresa
       LEFT JOIN vehiculos v ON v.id_vehiculo = mm.id_vehiculo
       ${whereSql}`,
    ...
    res.json({
      ...
      totalMonto: Number(sumRow?.total_monto || 0)
    });
  } catch (e) {
    console.error('reportes/mensualidades-pagos', e);
    res.status(500).json({ success: false, message: 'Error obteniendo pagos de mensualidades' });
  }
});
...
router.get('/export/xlsx', async (req, res) => {
  try {
    ...
    // Filtro por tipo: ahora usa código del tipo desde tipos_vehiculos
    if (tipo) { 
      // Puede ser código (carro, moto, bici) o id_tipo
    ...
    wsResumen.addRow(['Reporte de Movimientos - ParkSystem', '']);
    ...
    wsResumen.getCell('B4').numFmt = '[$$-es-CO] #,##0';
    wsResumen.getCell('B5').numFmt = '[$$-es-CO] #,##0';
    wsResumen.getCell('B7').numFmt = '[$$-es-CO] #,##0';
    ...
    // Totales por método: movimientos (ajustados) + mensualidades
    ...
    // Sumar pagos de mensualidades por método
    const [mensMetodos] = await pool.query(
      `SELECT metodo_pago, COALESCE(SUM(monto), 0) AS total
       FROM mensualidades_pagos
       WHERE id_empresa = ? AND DATE(fecha_pago) BETWEEN ? AND ?
       GROUP BY metodo_pago`,
      [idEmpresa, dDesde, dHasta]
    );
    mensMetodos.forEach(m => {
      const k = String(m.metodo_pago);
      if (k === 'efectivo') mTotals.efectivo += Number(m.total);
      else if (k === 'tarjeta') mTotals.tarjeta += Number(m.total);
      else if (k === 'QR' || k === 'transferencia') mTotals.QR += Number(m.total);
    });
    ...
    [rEf, rTa, rQr].forEach(r=>{ r.getCell(2).numFmt = '[$$-es-CO] #,##0'; });
    // Colores por método
    ...
    // Fila Total real (valor calculado, sin fórmula para evitar referencia circular)
    const lastDataRow = ws.rowCount + 1;
    const totalMovReal = rows.reduce((acc, r) => acc + Number(r.total_a_pagar || 0), 0);
    ws.getCell(`F${lastDataRow}`).value = 'Total real';
    ws.getCell(`G${lastDataRow}`).value = Math.round(totalMovReal);
    ws.getCell(`G${lastDataRow}`).numFmt = '[$$-es-CO] #,##0';
    ws.getCell(`F${lastDataRow}`).font = { bold: true };
    ws.getCell(`G${lastDataRow}`).font = { bold: true };

    // Hoja Mensualidades (pagos del período) — LEFT JOIN para incluir arriendos (id_vehiculo NULL)
    const [mensRows] = await pool.query(
      `SELECT DATE(mp.fecha_pago) as fecha, mp.metodo_pago, mp.monto,
              COALESCE(v.placa, mm.referencia_espacio) AS placa,
              mm.tipo_servicio, mp.id_mensualidad,
    ...
    wsMens.columns = [
      ...
      { header: 'Placa / Espacio', key: 'placa', width: 18 },
    ...
    // Fila total mensualidades
    const totalMensReal = mensRows.reduce((acc, r) => acc + Number(r.monto || 0), 0);
    const lastMensRow = wsMens.rowCount + 1;
    wsMens.getCell(`F${lastMensRow}`).value = 'Total';
    wsMens.getCell(`C${lastMensRow}`).value = Math.round(totalMensReal);
    wsMens.getCell(`C${lastMensRow}`).numFmt = '[$$-es-CO] #,##0';
    wsMens.getCell(`F${lastMensRow}`).font = { bold: true };
    wsMens.getCell(`C${lastMensRow}`).font = { bold: true };

    wsMens.getColumn('fecha').numFmt = 'dd/mm/yyyy';
    wsMens.getColumn('monto').numFmt = '[$$-es-CO] #,##0';
    wsMens.getRow(1).font = { bold: true };
    wsMens.getRow(1).eachCell(c => {
      c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0D6EFD' } };
      c.font = { color: { argb: 'FFFFFFFF' }, bold: true };
    });
    wsMens.autoFilter = { from: 'A1', to: 'I1' };
