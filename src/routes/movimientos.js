...
const { obtenerTarifaActiva, registrarIngreso } = require('../services/movimientoService');
...
// La lógica de negocio vive en src/services/movimientoService.js (compartida con /api/anpr/dahua)
router.post('/ingreso', verifyToken, async (req, res) => {
    try {
        const { placa, id_tipo, auto_tipo } = req.body;
        ...
        res.status(resultado.status).json({ success: true, data: resultado.data, message: resultado.message });
    } catch (error) {
